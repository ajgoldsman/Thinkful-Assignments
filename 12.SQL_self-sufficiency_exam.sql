...
--3. Write a query that returns summary statistics for avg_math_4_score by state. Make sure to sort the
--   results alphabetically by state name.
SELECT state, COUNT(avg_math_4_score) AS count, ROUND(AVG(avg_math_4_score),2) AS average, MIN(avg_math_4_score) AS min, MAX(avg_math_4_score) AS max
FROM naep
GROUP BY state
ORDER BY state;
...
--5. Write a query that returns a field called bottom_10_states that lists the states in the bottom 
...
HAVING avg_math_4_score < ROUND(AVG(avg_math_4_score),0)
ORDER BY avg_math_4_score;
--For some reason, this list doesn't include Washington DC, which was in the bottom_10 list.
...
SELECT naep.state, ROUND(avg_math_4_score,2) AS avg_math_4_score, ROUND(total_expenditure,0) AS total_expenditure
FROM naep LEFT OUTER JOIN finance
ON naep.id = finance.id
WHERE naep.year = 2000
GROUP BY naep.state, avg_math_4_score, total_expenditure
HAVING avg_math_4_score IS NOT NULL
